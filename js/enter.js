...
const newspaperform = document.querySelector('#newspaperform');
...
const newspapers_form = document.querySelector('#newspapers');
// show the DIV if it is hidden
newspapers_form.style.display = 'block';


// event handler
newspaperform.onsubmit = (e) => {
    // gets the names and current values from the form
    const formData = new FormData(newspaperform);
    // FETCH
    fetch('enter.php', {
        method: "POST",
        body: formData,
    ...
    .then( (data) => {
        newspapers_form.style.display = 'none';
